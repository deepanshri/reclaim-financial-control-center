...
def validate_datasets():
    ...
    for p in periods:
        ...
        # Merchant-facing severity from Money Affected:
        # ₹0–₹99,999 MONITOR | ₹1,00,000–₹2,99,999 ACTION NEEDED | ₹3,00,000+ URGENT ACTION
        if confirmed_loss < 100000.0:
            computed_status = "MONITOR"
            severities_found.add("MONITOR")
        elif confirmed_loss < 300000.0:
            computed_status = "ACTION NEEDED"
            severities_found.add("ACTION NEEDED")
        else:
            computed_status = "URGENT ACTION"
            severities_found.add("URGENT ACTION")
        potential_recovery = sum(
            float(a.get("recoverable_amount") or 0)
            for a in confirmed_anomalies
            if str(a.get("is_recovery_eligible", "true")).lower() in ("true", "1", "yes")
        )
            
        assert potential_recovery <= confirmed_loss + 0.01, f"Rule 19: Potential recovery {potential_recovery} cannot exceed Money Affected {confirmed_loss}"
        ...
        for req in recovery_requests:
            ...
            if status in ["resolved", "recovered"]:
                ...
                remainder = amt_req - amt_rec
                if remainder > 0.009:
                    not_recovered_requested += remainder
        ...
        outcome_sum = total_recovered + under_review_requested + not_recovered_requested
        assert abs(outcome_sum - total_requested) < 0.02, (
            f"Rule 17b: Recovered + Under Review + Not Recovered ({outcome_sum}) "
            f"must equal requested ({total_requested}) in {p_key}"
        )
    ...
    # RULE 20: All 3 merchant-facing severity tiers represented
    assert "MONITOR" in severities_found, "Missing MONITOR severity in dataset"
    assert "ACTION NEEDED" in severities_found, "Missing ACTION NEEDED severity in dataset"
    assert "URGENT ACTION" in severities_found, "Missing URGENT ACTION severity in dataset"
