...
2. Anomaly-level recovery eligibility with documented business justification:
   - fee_rate_increase             -> 80% (MDR dispute ~80% industry approval rate)
   - duplicate_refund              -> 95% (trace-supported, near-certain recovery)
   - missing_settlement            -> 85% (UTR traceable, bank cooperation required)
   - bank_credit_missing (IFSC)    -> 0%  (bank operational return, non-disputable)
   - bank_credit_missing (normal)  -> 70% (bank trace required, partial recovery typical)
   - settlement_amount_discrepancy -> 75% (batch dispute, partial adjustments common)
   - settlement_delay              -> 0%  (no direct monetary loss)
   - uncredited_refund             -> 85% (customer trace + gateway record)
3. Severity Model:
   - INR 0 - 99,999       -> REVIEW (Green)
...
def generate_period_dataset(p_key: str):
    ...
    rng = random.Random(42 + abs(hash(p_key)) % 100000)
    p_dir = ROOT / p_key
    ...
    for anom in cfg["anomalies_def"]:
        if anom["type"] == "bank_credit_missing" and anom["status"] == "confirmed":
            for ev in anom["evidence"]:
                ...
                if "set_" in tid:
                    bc_missing_settlement_ids.add(tid)
    ...
    s_fields = [
        ...
        "settlement_date","transaction_count","refund_adjustment"
    ]
    ...
    for settl in settlements:
        ...
        is_bc_missing = s_id in bc_missing_settlement_ids
        if is_bc_missing:
            bc_idx += 1
            continue
