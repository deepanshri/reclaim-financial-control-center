...
class DataRepository:
    ...
    def _load_unlocked(self, force_reload: bool = False) -> None:
        ...
        logger.info(f"Loading and normalizing records from {self.data_dir}")
        self._periods = {}
        ...
        logger.info(f"DataRepository normalized {len(self._periods)} periods successfully.")
    ...
    def get_merchant_data(self, period: Optional[str] = None) -> Dict[str, Any]:
        ...
        meta = store.meta
        fee_rate = 0.018
        if store.fee_contracts:
            fee_rate = store.fee_contracts[0].contracted_rate
    ...
    def get_payment_by_id(self, payment_id: str, period: Optional[str] = None) -> Optional[Payment]:
        ...
        for store in self._periods.values():
